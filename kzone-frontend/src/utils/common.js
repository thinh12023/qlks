...
const getTienPhongChiTiet = ({ type, payload }) => {
  let tienPhongChiTiet;
  switch (type) {
    case LOAI_THUE_PHONG.QUA_DEM:
      tienPhongChiTiet = getByOvernight(payload);
      break;
    case LOAI_THUE_PHONG.THEO_GIO:
      tienPhongChiTiet = getByHours(payload);
      break;
    case LOAI_THUE_PHONG.THEO_NGAY:
      tienPhongChiTiet = getByDaily(payload);
      break;
    default:
      tienPhongChiTiet = [];
      break;
...
const getByDaily = ({
  ...
  if (
    ...
    const total = ds?.reduce((acc, item) => {
      acc = parseFloat(acc) + parseFloat(item?.cost);
      return acc;
    }, 0);
...
const getByHours = ({
  ...
  ds.push({
    fromDate: `${checkoutDate} ${pointCheckoutDayTime}`,
    toDate: `${checkoutDate} ${checkoutTime}`,
    cost: hourlyRate * tongThoiGianThuePhong,
