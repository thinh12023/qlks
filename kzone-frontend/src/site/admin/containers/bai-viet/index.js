...
function BaiViet({
  ...
  function confirmHotNews(item) {
    let payload = {
      isHotNews:1
    };
    onChangeHotNews({ id: item.id ,payload });
  }

  function cancelHotNews(item) {
    let payload = {
      isHotNews:0
    };
    onChangeHotNews({ id: item.id ,payload });
  ...
  const onClickAction = ({ type = "add", payload = {} }) => (e) => {
    switch (type) {
      ...
      case "delete":
        ...
        break;
      case "changeHotNews":
        confirmHotNews(payload);
        break;
      case "cancelHotNews":
        cancelHotNews(payload);
        break;
      case "edit":
        if (props.history)
          props.history.push(
