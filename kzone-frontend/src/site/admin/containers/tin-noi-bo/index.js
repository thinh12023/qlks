...
function TinNoiBo({
  ...
  let idsTinNoiBo = dsTinNoiBo?.map((i) => i.id);
  ...
  const onClickAction = ({ type = "add", payload = {} }) => (e) => {
    switch (type) {
      case "add":
        if (props.history) {
          props.history.push("/admin/tin-noi-bo/tao-moi");
        } else window.location.href = "/admin/tin-noi-bo/tao-moi";
        break;
      case "delete":
        showConfirm(payload);
        break;
      case "edit":
        if (props.history)
          props.history.push(
            "/admin/tin-noi-bo/" + payload.id + "?mode=edit"
          );
        else
      ...
      case "view":
        if (props.history)
          props.history.push(
            "/admin/tin-noi-bo/" + payload.id + "?mode=view"
          );
        else
          window.location.href =
            "/admin/tin-noi-bo/" + payload.id + "?mode=view";
    ...
    if (!e?.target?.checked) {
      setState({
        selectedItem: state.selectedItem.filter(
          (item, index) => !idsTinNoiBo.includes(item)
        ),
      });
    } else {
      setState({
        selectedItem: [...state.selectedItem, ...idsTinNoiBo].filter(
